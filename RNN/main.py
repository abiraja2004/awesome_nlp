...
if __name__ == "__main__":
    ...
    pickle.dump(list(w2i.items()), open(os.path.dirname(
        os.path.realpath(__file__)) +
        "/models/w2i.pickle", 'wb'))
    pickle.dump(list(i2w.items()), open(os.path.dirname(
        os.path.realpath(__file__)) +
    ...
    decoder = Elman_Decoder(dim, len(w2i), embed, args.length, enable_cuda)
